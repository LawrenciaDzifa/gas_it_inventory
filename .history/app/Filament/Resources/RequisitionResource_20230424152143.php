    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                ...
                // wrap the status column in labels such that the status is displayed as a badge, where approves is green, pending is yellow and declined is red, using getStateUsing method and it should be a rawColumn
                TextColumn::make('status')
                    ->getStateUsing(function (Model $record) {
                        if ($record->status == 'approved') {
                            return '<span class="bg-blue-500 rounded px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">Approved</span>';
