    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                ...
                // wrap the type column in a badge such that the type is displayed as a badge, where the badge color is green when the type is initial stock, and yellow when the type is restock
                TextColumn::make('type')->getStateUsing(function (Model $record) {
                    if ($record->type == 'initial stock') {
                        return '<span class="badge badge-success">' . $record->type . '</span>';
                    } elseif($record->type == 'restock') {
                        return '<span class="badge badge-warning">' . $record->type . '</span>';
                    }
                }),
